...
class QueryPlanningTraversal {
    constructor(parameters, selectionSet, startFetchIdGen, hasDefers, rootKind, costFunction, initialContext, excludedDestinations = [], excludedConditions = []) {
        var _a;
    ...
    resolveConditionPlan(edge, context, excludedDestinations, excludedConditions) {
        const bestPlan = new QueryPlanningTraversal({
            ...this.parameters,
            root: edge.head,
        }, edge.conditions, 0, false, 'query', this.costFunction, context, excludedDestinations, (0, query_graphs_1.addConditionExclusion)(excludedConditions, edge.conditions)).findBestPlan();
...
class FetchGroup {
    constructor(dependencyGraph, index, subgraphName, rootKind, parentType, isEntityFetch, _selection, _inputs, mergeAt, deferRef, subgraphAndMergeAtKey, cachedCost, generateQueryFragments = false, isKnownUseful = false, inputRewrites = []) {
    ...
    cloneShallow(newDependencyGraph) {
        var _a;
        return new FetchGroup(newDependencyGraph, this.index, this.subgraphName, this.rootKind, this.parentType, this.isEntityFetch, this._selection.clone(), (_a = this._inputs) === null || _a === void 0 ? void 0 : _a.clone(), this.mergeAt, this.deferRef, this.subgraphAndMergeAtKey, this.cachedCost, this.generateQueryFragments, this.isKnownUseful, [...this.inputRewrites]);
    }
    cost() {
        if (!this.cachedCost) {
...
class QueryPlanner {
    ...
    collectAllOverrideLabels() {
        var _a;
        this._defaultOverrideConditions = new Map((_a = this.supergraph.schema.directives()
            .find((d) => d.name === 'join__field')) === null || _a === void 0 ? void 0 : _a.applications().map((application) => application.arguments().overrideLabel).filter(Boolean).map(label => [label, false]));
    }
    ...
    withSiblingTypenameOptimizedAway(operation) {
        ...
        return new federation_internals_1.Operation(operation.schema, operation.rootKind, updatedSelectionSet, operation.variableDefinitions, operation.fragments, operation.name);
    }
...
function withoutIntrospection(operation) {
    ...
    return new federation_internals_1.Operation(operation.schema, operation.rootKind, operation.selectionSet.lazyMap((s) => isIntrospectionSelection(s) ? undefined : s), operation.variableDefinitions, operation.fragments, operation.name);
}
...
function computeRootSerialDependencyGraph(parameters, hasDefers) {
    ...
    for (let i = 1; i < splittedRoots.length; i++) {
        ...
        if (prevSubgraph === newSubgraph) {
            ...
            prevDepGraph = computeRootFetchGroups(FetchDependencyGraph.create(supergraphSchema, federatedQueryGraph, startingFetchId, rootType, parameters.config.generateQueryFragments), prevPaths, root.rootKind);
        }
        else {
...
function operationForEntitiesFetch(subgraphSchema, selectionSet, allVariableDefinitions, operationName) {
    ...
    return new federation_internals_1.Operation(subgraphSchema, 'query', entitiesCall, variableDefinitions, undefined, operationName);
}
function operationForQueryFetch(subgraphSchema, rootKind, selectionSet, allVariableDefinitions, operationName) {
    return new federation_internals_1.Operation(subgraphSchema, rootKind, selectionSet, allVariableDefinitions.filter(selectionSet.usedVariables()), undefined, operationName);
}
